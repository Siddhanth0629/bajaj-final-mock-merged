...
function getTargetElements(target) {
  ...
  return toArray(elements).reduce(function (result, element) {
    return [].concat(
      ...
      _toConsumableArray(toArray(element).filter(isNode))
    );
  }, []);
}
...
var reUnicode = RegExp(
  "".concat(rsFitz, "(?=").concat(rsFitz, ")|").concat(rsSymbol).concat(rsSeq),
  "g"
);
  ...
  if (settings.absolute) {
    ...
    toArray(nodes).forEach(function (node) {
      ...
      node.style.left = isLine
        ...
        : "".concat(left - (isChildOfLineNode ? contentBox.left : 0), "px"); // Set the height of the current node to the cached value.
      ...
      node.style.width = isLine
        ...
        : "".concat(width, "px"); // Finally, set the node's position to absolute.
